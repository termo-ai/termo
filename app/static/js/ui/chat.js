...
function handleMessageChunk(content) {
  if (
    !currentMessageDiv ||
    (isCodeBlockActive && !content.trim().startsWith("```"))
  ) {
...
function handleCodeChunk(code, language, start, end) {
  if (!currentCodeBlock || !isCodeBlockActive) {
    ...
    currentCodeBlock.innerHTML = `
            ...
            <pre class="p-4 m-0 overflow-x-auto"><code class="language-${
              language || "shell"
            }"></code></pre>
        `;
...
function updateCodeBlockStatus(codeBlock, status) {
  ...
  if (statusElement) {
    statusElement.className =
      "code-execution-status text-sm px-2 py-0.5 rounded-md";
...
function handleOutputChunk(output) {
  if (!currentOutputBlock) {
    ...
    header.className =
      "flex justify-between items-center px-4 py-2 bg-gray-800 rounded-t-lg";
    header.innerHTML = `
    ...
    contentContainer.className =
      "output-content max-h-60 overflow-y-auto transition-all duration-200";
...
function createMessageGroup(role) {
  ...
  div.className = `rounded-lg p-4 ${
    role === "user" ? "bg-gray-700" : "bg-gray-600"
  }`;
  div.innerHTML = `
...
function exportChat() {
  const messages = Array.from(messageContainer.children).map((msg) => {
    ...
    const output = Array.from(msg.querySelectorAll(".whitespace-pre-wrap"))
      .map((o) => o.textContent)
      .join("\n");
